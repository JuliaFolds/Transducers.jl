...

{
    ...
    "title": "Covariance",
    "category": "section",
    "text": "Transducer xf_mdot above can also be used to compute the covariance.  First, we need the number of pairs of elements in xs and ys that both of them are not missing:nonmissings = mapfoldl(xf_mdot |> Count(), right, zip(xs, ys); init=0)@assert nonmissings == 2  # hideWe do this by using Count and right.  Count ignores input and count the number of times the input is provided. Since xf_mdot provides the inputs to the downstream transducer, this correctly counts the number of non-missing pairs.  Function right is simply defined as right(l, r) = r (and right(r) = r). Thus, the whole mapfoldl returns the last output of Count.  In case Count never gets called (i.e., there are no non-missing pairs), we pass init=0.ans =  # hide\nmapfoldl(xf_mdot |> Count(), right, zip(Int[], Int[]); init=0)@assert ans == 0  # hideFinally, we have to pre-process the input to xf_mdot by subtracting the average.  It\'s easy to do with Map:using Statistics: mean\n\nfunction xf_demean(xs, ys)\n    xmean = mean(skipmissing(xs))\n    ymean = mean(skipmissing(ys))\n    return Map(((x, y),) -> (x - xmean, y - ymean))\nend\n\nmapfoldl(xf_demean(xs, ys) |> xf_mdot, +, zip(xs, ys)) / nonmissings"
},
